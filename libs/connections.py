...
def conv2d(x, n_filters,
    ...
    with tf.variable_scope(name):
        ...
        if pool_size:
            #conv = tf.layers.max_pooling2d(inputs=conv, pool_size=pool_size, strides=pool_stride)
            conv = tf.nn.avg_pool(conv, pool_size, pool_stride, padding=padding)

        print "name =", name, conv.get_shape()
        return conv
